...
class TestCalcBmrMifflin:
    """_calc_bmr_mifflin() の計算ロジックを検証する"""

    def test_male_bmr(self):
        """男性: 10*70 + 6.25*170 - 5*30 + 5 = 700+1062.5-150+5 = 1617.5 → 1618"""
        result = database._calc_bmr_mifflin(70.0, 170.0, 30, "male")
        assert result == 1618

    def test_female_bmr(self):
        """女性: 10*60 + 6.25*160 - 5*25 - 161 = 600+1000-125-161 = 1314"""
        result = database._calc_bmr_mifflin(60.0, 160.0, 25, "female")
        assert result == 1314

    def test_weight_zero_returns_none(self):
        """weight_kg=0 は None を返すこと（0はfalsyのため）"""
        result = database._calc_bmr_mifflin(0, 170.0, 30, "male")
        assert result is None

    def test_weight_none_returns_none(self):
        """weight_kg=None は None を返すこと"""
        result = database._calc_bmr_mifflin(None, 170.0, 30, "male")
        assert result is None

    def test_height_none_returns_none(self):
        """height_cm=None は None を返すこと"""
        result = database._calc_bmr_mifflin(70.0, None, 30, "male")
        assert result is None

    def test_age_none_returns_none(self):
        """age=None は None を返すこと"""
        result = database._calc_bmr_mifflin(70.0, 170.0, None, "male")
        assert result is None

    def test_unknown_gender_returns_none(self):
        """gender が male/female 以外は None を返すこと"""
        result = database._calc_bmr_mifflin(70.0, 170.0, 30, "other")
        assert result is None

    def test_empty_gender_returns_none(self):
        """gender が空文字列は None を返すこと"""
        result = database._calc_bmr_mifflin(70.0, 170.0, 30, "")
        assert result is None
